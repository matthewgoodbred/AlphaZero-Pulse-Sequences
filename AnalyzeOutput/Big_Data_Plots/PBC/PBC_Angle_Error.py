...
This is to make some plots of rewards versus angle error.
-By pbc
...
for val in range(5):
    ...
    for sequence in ps_dict:
        for idx, ps_obj in enumerate(ps_dict[sequence]):
            ...
            if no_err_reward == 200:        # For now
                no_err_reward = 10
                # print(ps_obj.get_ps())
            aerss = ps_obj.get_rot_error_fids()
            ...
            if pbc == 2.0:
                ...
                pbc2y.append(aerss[val])
            if pbc == 3.0:
                pbc3x.append(no_err_reward)
                pbc3y.append(aerss[val])
            if pbc == 4.0:
                pbc4x.append(no_err_reward)
                pbc4y.append(aerss[val])
            if pbc == 5.0:
                pbc5x.append(no_err_reward)
                pbc5y.append(aerss[val])
    ...
    additional = 'PBC Angle= ' + str(angle_vals[val])
    ...
    plt.ylabel('Reward (fractional rotation error= ' + str(angle_vals[val]) + ')', fontsize=12)    # TODO
    plt.xlim(-0.5, 5)
    plt.ylim(-0.5, 5)
    # plt.show()
