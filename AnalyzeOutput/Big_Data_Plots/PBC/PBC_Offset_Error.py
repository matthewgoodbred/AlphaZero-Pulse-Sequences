...
This is to make some plots of rewards versus offset error.
-By pbc
...
for val in range(9):
    ...
    for sequence in ps_dict:
        for idx, ps_obj in enumerate(ps_dict[sequence]):
            ...
            if pbc == 2.0:
                ...
                pbc2y.append(offs[val])
            if pbc == 3.0:
                pbc3x.append(no_err_reward)
                pbc3y.append(offs[val])
            if pbc == 4.0:
                pbc4x.append(no_err_reward)
                pbc4y.append(offs[val])
            if pbc == 5.0:
                pbc5x.append(no_err_reward)
                pbc5y.append(offs[val])
    ...
    additional = 'PBC Offset= ' + str(off_vals[val])
