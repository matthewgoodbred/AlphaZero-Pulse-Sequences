...
# TODO: This specific version of pulse_sequences does not use rng to evaluate pulse sequences; no stochastic nature
'''
...
def get_Hsys(N, cs_strength=1, offset=0, dipolar_strength=1e2,
             rng=None, return_all=False, stochastic=False):
    """
    ...
    if stochastic:
        ...
        chemical_shifts = np.full((N,), cs_strength)
    ...
    if stochastic:
        ...
        dipolar_matrix = np.full((N, N), dipolar_strength)
...
class PulseSequenceConfig(object):
    ...
    def __init__(self,
        ...
        self.stochastic = False if stochastic is None else stochastic     # Defaults to no stochastic nature
            ...
            for _ in range(ensemble_size):
                ...
                if offset_error > 0:
                    ...
                    if stochastic:
                        ...
                        o = offset_error
                elif offset_error < 0:      # For two-sided plots (log positive and negative)
                    o = offset_error
                if save_name is not None:
                    H, (cs, offset, dip) = get_Hsys(
                        ...
                        offset=o,
                        rng=self.rng, return_all=True, stochastic=self.stochastic)
                    ...
                    H = get_Hsys(N=N,
                                 dipolar_strength=dipolar_strength,
                                 offset=o,
                                 rng=self.rng, stochastic=self.stochastic)
    ...
    def apply(self, pulse):
        ...
        # if pulse != 0:      # Not including delays in sequences
